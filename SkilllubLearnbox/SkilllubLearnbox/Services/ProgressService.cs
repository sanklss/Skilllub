...
public class ProgressService
{
    ...
    public async Task MarkPracticeAsCompletedAsync(string userId, string lessonId, int score = 100)
    {
        try
        {
            ...
            if (!progress.PracticeCompleted || score > progress.BestScore)
            {
                ...
                await _client.From<UserProgress>().Update(progress);
                _logger.LogInformation("✅ Практика урока {LessonId} отмечена как выполненная", lessonId);
        ...
        {
            _logger.LogError(ex, "❌ Ошибка при отметке практики урока");
        }
    }
    ...
    public async Task<LessonStatusDto> GetLessonStatusAsync(string userId, string lessonId)
    {
        try
        {
            ...
            return new LessonStatusDto
            {
                ...
                HasQuiz = lessonInfo?.HasQuiz ?? false,
                HasCodeExercise = lessonInfo?.HasCodeExercise ?? false
            };
        }
        catch (Exception ex)
        {
    ...
    private async Task CheckAndUpdateLessonCompletionAsync(string userId, string lessonId)
    {
        try
        {
            ...
            if (lessonInfo.Value.HasQuiz && lessonInfo.Value.HasCodeExercise)
            {
                shouldBeCompleted = progress.TheoryCompleted && progress.PracticeCompleted;
            }
            else if (lessonInfo.Value.HasQuiz)
            {
                shouldBeCompleted = progress.PracticeCompleted;
            }
            else if (lessonInfo.Value.HasCodeExercise)
            {
                shouldBeCompleted = progress.PracticeCompleted;
            }
            else
            {
                shouldBeCompleted = progress.TheoryCompleted;
            }
        ...
        try
        {
            ...
            foreach (var lessonId in lessonIds)
            {
                ...
                if (userProgress != null)
                {
                    ...
                    userProgress.PracticeCompleted = false;
